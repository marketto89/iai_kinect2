...
private:
  ...
  libfreenect2::CudaPacketPipeline *packetPipeline;
...
public:
      ...
      deltaT(1.0 / fps), topics(COUNT + 1)
  {
    ...
    packetPipeline = new libfreenect2::CudaPacketPipeline(deviceIdDepth);
#else
    packetPipeline = new libfreenect2::CudaPacketPipeline(deviceIdDepth);
#endif
  ...
  void run()
  {
    ...
    infos.resize(COUNT);
    statusPubs.resize(COUNT + 1); //+ 1 for the cloud topic
...
private:
  ...
  void equalizeImage(cv::Mat& input_image, cv::Mat& output_image)
  {
    ...
    double max_less_min = maxVal - minVal;
    for (int i = 0; i < input_image.rows; i++)
    {
      for (int j = 0; j < input_image.cols; j++)
      {
        output_image.at<unsigned char>(i,j) = (unsigned char) (double(input_image.at<uint16_t>(i,j) - minVal) * 255 / max_less_min );
      }
    }
  ...
  void createTopicNames()
  {
    ...
    topics[COUNT] = "/" + cameraName + "/" K2_TOPIC_CLOUD_IR;
  }
};
